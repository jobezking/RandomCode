...
# Get total logical cores (threads)
logical_cores = os.cpu_count() or 4 # Default to 4 if detection fails

# Get physical cores (standard approach)
# Note: psutil is better for this, but to avoid new dependencies, 
# we usually assume physical = logical / 2 on hyperthreaded machines
physical_cores = logical_cores // 2 if logical_cores > 2 else 1

print(f"Hardware Detected: {logical_cores} threads, {physical_cores} physical cores.")

# Intra-op: Parallelism within an op (best at thread count)
tf.config.threading.set_intra_op_parallelism_threads(logical_cores)

# Inter-op: Parallelism between independent ops (best at physical core count)
tf.config.threading.set_inter_op_parallelism_threads(physical_cores)

# Optimize the Data Pipeline as well
